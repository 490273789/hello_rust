pub fn quote_borrow() {
    ...
    // 2、写（W）：数据可以被修改
    // 3、拥有（O）：数据可以被移动或释放
    // 4、流动权限F： 在表达式使用输入引用或返回引用是需要
    // F权限在函数体内不会发生变化，如果一个引用被允许在特定表达式中使用（即流动），那么它就具有F权限
    ...
    // 权限是定义在位置上的（不仅仅是变量），任何可以放在赋值语句左侧的东西

    // 不可变引用（共享引用）：只读的
    // 可变引用（独占引用）：在不宜动数据的情况下，临时提供可变访问
