pub fn ownership_mod() {
  ...
  // 所有权：确保rust程序安全的一种机制，所有去看作为内存安全的规范
  // 安全：程序中没有未定义的行为
  // 解决的问题
  // 所有的程序都必须和计算机内存打交道，如何从内存中申请空间来存放程序的运行内容，如何在不需要的时候释放这些空间
  // stack frame有rust自动管理，不允许手动管理内存
  // 如果一个变量拥有一个引用类型地址，当rust释放该变量的frame时，rust会释放该指针指向的的堆内存
  // rust的引用类型基本都是使用Box实现的
  // 移动堆数据原则： 如果变量x将堆数据的所有权移动给另一个变量y，那么咋哦移动后，x不能在使用。
  // 避免数据移动的一种方式是使用.clone()方法进行克隆
